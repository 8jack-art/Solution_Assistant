/**
 * 财务计算指标表JSON生成测试文件（V2 - 严格按前端modal逻辑）
 * 测试 investment、investmentEfficiency、solvency 字段的正确性
 */

import { buildFinancialIndicatorsJSON } from './dist/utils/tableDataBuilders/buildFinancialIndicators.js';

// 模拟测试数据 - 参考用户提供的实际JSON
...
console.log("财务计算指标表JSON生成测试（V2 - 严格按前端modal逻辑）");
console.log("=".repeat(60));

// 执行生成
const jsonResult = buildFinancialIndicatorsJSON(mockIndicatorsData);
...
const investmentTests = [
  { name: "总投资", value: result.investment.totalInvestment, expected: 7000 },

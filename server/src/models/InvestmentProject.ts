...
export class InvestmentProjectModel {
  static async findById(id: string): Promise<InvestmentProject | null> {
    try {
  ...
  static async findByUserId(userId: string, isAdmin = false): Promise<InvestmentProject[]> {
    try {
  ...
  static async create(projectData: Omit<InvestmentProject, 'id' | 'created_at' | 'updated_at'>): Promise<InvestmentProject | null> {
    try {
      // 生成UUID
      const id = randomUUID()
      ...
      const [result] = await pool.execute(
        `INSERT INTO investment_projects 
         (id, user_id, project_name, total_investment, project_info, status, 
          ...
          land_lease_area, land_lease_unit_price, land_purchase_area, land_purchase_unit_price, seedling_compensation) 
         VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`,
        [
          id,
          projectData.user_id,
  ...
  static async update(id: string, updates: Partial<InvestmentProject>): Promise<InvestmentProject | null> {
    try {
      ...
      await pool.execute(
        `UPDATE investment_projects SET ${setClause} WHERE id = ?`,
        [...values, id]
      )
  ...
  static async updateStatus(id: string, status: 'draft' | 'completed'): Promise<InvestmentProject | null> {
    try {
      await pool.execute(
        'UPDATE investment_projects SET status = ? WHERE id = ?',
        [status, id]
      )
  ...
  static async lock(id: string): Promise<InvestmentProject | null> {
    try {
      await pool.execute(
        'UPDATE investment_projects SET is_locked = TRUE, locked_at = CURRENT_TIMESTAMP WHERE id = ?',
        [id]
      )
  ...
  static async unlock(id: string): Promise<InvestmentProject | null> {
    try {
      await pool.execute(
        'UPDATE investment_projects SET is_locked = FALSE, locked_at = NULL WHERE id = ?',
        [id]
      )
  ...
  static async delete(id: string): Promise<boolean> {
    try {
      const [result] = await pool.execute(
        'DELETE FROM investment_projects WHERE id = ?',
        [id]
      ) as any[]

...
// 评估可行性研究报告分档数据
const FEASIBILITY_EVAL_BRACKETS = [
    { threshold: 500, min: 0.8, max: 1.2 },
    { threshold: 1000, min: 1.2, max: 2.0 },
 ...
 * 计算单个前期咨询子项费用
 */
function calculateConsultingItemFee(totalFunding, brackets) {
    if (totalFunding <= 0)
        return 0;
    // 找到所在区间
 ...
 */
export function calculatePreliminaryConsultingFee(totalFunding) {
    if (totalFunding <= 0)
        return 0;
 ...
 */
export function calculateEnvironmentalReportFee(totalFunding) {
    if (totalFunding <= 0)
        return 0;
    // 将万元转换为亿元

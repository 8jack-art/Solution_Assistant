...
export class InvestmentEstimateModel {
    ...
    static async create(estimateData) {
        try {
            // 生成UUID
            const id = randomUUID();
            const [result] = await pool.execute(`INSERT INTO investment_estimates 
    ...
    static async update(id, updates) {
        try {
            ...
            await pool.execute(`UPDATE investment_estimates SET ${setClause} WHERE id = ?`, [...values, id]);
            return await this.findById(id);
        }
        catch (error) {
    ...
    static async delete(id) {
        try {
            const [result] = await pool.execute('DELETE FROM investment_estimates WHERE id = ?', [id]);
...
//# sourceMappingURL=InvestmentEstimate.js.map
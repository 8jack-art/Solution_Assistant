  ...
  loadProjectData: async () => {
    ...
    try {
      ...
      const variables: ReportVariable[] = [
        ...
        // 【新增】构建 management_expenses 变量（管理费用）
        { key: '{{management_expenses}}', label: '管理费用', value: (() => {
          ...
          if (totalCostData.yearlyData && Array.isArray(totalCostData.yearlyData)) {
            ...
            // 从summary中获取年均管理费、运营期合计和费用名称
            const summary = totalCostData.summary || {}
            
            // 构建完整的JSON数据
            const fullManagementData = {
              name: summary.费用名称 || '管理费用',
              yearlyData: managementData,
              年均管理费: summary.年均管理费 || 0,
              运营期合计: summary.运营期合计 || 0
            }
            
            return JSON.stringify(fullManagementData, null, 2)
          }

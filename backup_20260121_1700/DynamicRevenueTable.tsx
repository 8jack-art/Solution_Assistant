...
const DynamicRevenueTable: React.FC<DynamicRevenueTableProps> = ({ deductibleInputTax = 0, depreciationData = [] }) => {
  ...
  const isSavingRef = useRef(false) // 防止重复保存触发弹窗
  ...
  }, [aiAnalysisResult, context]) // 只监听aiAnalysisResult和context的变化，避免循环
  ...
  }, [revenueItems]) // 监听revenueItems的变化
  ...
  useEffect(() => {
    ...
    // 显示 revenueTableData 的完整数据
    if (revenueTableData) {
      console.log('📋 [revenueTableData] 完整数据:', JSON.stringify(revenueTableData, null, 2));
    }
  ...
  }, [revenueItems, context, revenueTableData]);
  ...
  /**
   * 计算进项税额（固定资产待抵扣）的运营期列值
   * 逻辑说明：
   * 1. 当"销项税额 - 进项税额"大于0时，使用需要的值或剩余待抵扣税的较小值
   * 2. 当"销项税额 - 进项税额"小于等于0时，将负值的绝对值B1累加到待抵扣进项税中，当年增值税设为0
   */
  ...
  const handleExportRevenueTable = () => {
    ...
    import('xlsx-js-style').then((XLSX) => {
      ...
      years.forEach((year) => {
        ...
        // 增值税 = 销项税额 - 进项税额 - 进项税额（固定资产待抵扣）
        // 确保增值税不为负数
        const yearVat = Math.max(0, yearOutputTax - yearInputTax - yearFixedAssetInputTax);
                  ...
                  <Table.Tr>
                    ...
                    <Table.Td style={{ textAlign: 'center', border: '1px solid #dee2e6' }}>
                      {(() => {
                        ...
                        return totalEducationTax.toFixed(2);
                      })()}
                    </Table.Td>
                    {years.map((year) => {
                      // 使用新的增值税计算函数

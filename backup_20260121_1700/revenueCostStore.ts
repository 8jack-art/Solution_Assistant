...
export interface CostConfig {
  ...
  otherExpenses: {
    type: 'percentage' | 'directAmount';
    expenseType?: 'directAmount' | 'landTransfer'; // 费用类型: directAmount=直接填金额, landTransfer=土地流转费
    name?: string;
    percentage?: number;
    directAmount?: number;
    acreage?: number; // 亩数（土地流转费类型使用）
    unitPrice?: number; // 单价（万元/亩，土地流转费类型使用）
    taxRate?: number;
    applyProductionRate: boolean;
    remark?: string;
  };
...
interface RevenueCostState {
  ...
  // 财务指标数据管理
  setFinancialIndicators: (data: any) => void
      ...
      saveToBackend: async () => {
        ...
        try {
          ...
          const modelData = {
            ...
            financialIndicators: state.financialIndicators,
            // 添加折旧与摊销数据

...
async function testSSE() {
  ...
  try {
    ...
    // 先创建一个项目
    const createProjectResponse = await fetch('http://localhost:3001/api/investment-projects', {
      method: 'POST',
      headers: {
        'Content-Type': 'application/json',
      ...
      body: JSON.stringify({
        project_name: '测试项目-' + Date.now(),
        project_type: 'real_estate',
        total_investment: 1000000,
        construction_period: 24,
        description: '测试投资方案报告生成的项目'
      })
    });
    
    const projectData = await createProjectResponse.json();
    if (!projectData.success) {
      throw new Error('创建项目失败: ' + projectData.error);
    }

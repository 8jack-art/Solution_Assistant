...
const DynamicCostTable: React.FC<DynamicCostTableProps> = ({
  ...
  const generateCostTableData = () => {
    ...
    if (!context) {
      console.log('[DEBUG] generateCostTableData: context 为空，返回 null');
      return null;
    }
    
    const operationYears = context.operationYears;
    const years = Array.from({ length: operationYears }, (_, i) => i + 1);
    console.log('[DEBUG] generateCostTableData: 运营期年份:', years);
    ...
    years.forEach((year) => {
      ...
      console.log(`  第${year}年营业成本小计:`, yearTotal);
      row1.运营期.push(yearTotal);
      row1.合计 += yearTotal;
    });
    console.log('[DEBUG] 营业成本总计:', row1.合计);
    rows.push(row1);
  ...
  const handleExportCostTable = () => {
    console.log('[DEBUG] handleExportCostTable 开始导出:');
    console.log('[DEBUG] 当前达产率配置:', productionRates?.map(p => ({ year: p.yearIndex, rate: p.rate })) || 'undefined');
      ...
      years.forEach((year) => {
        ...
        // 修理费不应用达产率
        total += yearRepair;
        console.log(`🔍 Excel营业成本年份${year} - 1.4修理费:`, yearRepair);
  ...
  const calculateWagesTotal = useCallback((targetYear?: number, yearsArray?: number[]) => {
    ...
    if (targetYear !== undefined) {
      ...
      console.log('[DEBUG] calculateWagesTotal 单年结果:', {
        targetYear,
        yearWages,
        hasItems: costConfig.wages.items?.length > 0
      });
  ...
  }, [costConfig.wages, productionRates]); // 添加productionRates到依赖数组

import { ZhipuAIServiceFixed } from './src/services/zhipuServiceFixed.js'
import { LLMConfig } from './src/types/index.js'

async function testFixedZhipuService() {
  console.log('🔧 测试修复版本的智谱AI服务')
  ...
  try {
    ...
    console.log('\n🚀 调用修复版本的testConnection...')
    
    // 调用修复版本的服务方法
    const result = await ZhipuAIServiceFixed.testConnection(config)
...
// 测试多个模型
async function testMultipleModelsFixed() {
  console.log('\n\n🔄 修复版本测试多个模型')
  console.log('='.repeat(70))
...
async function main() {
  try {
    await testFixedZhipuService()
    await testMultipleModelsFixed()
  } catch (error) {
    console.error('\n💥 主测试流程异常:', error)
  }

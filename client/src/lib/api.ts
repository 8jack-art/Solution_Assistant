...
export const reportApi = {
  generate: (params: {
    ...
    api.post<any, ApiResponse<{ report_id: string }>>('/report/generate', params),

  stream: (reportId: string) =>
    new EventSource(`/api/report/stream/${reportId}`),

  getById: (id: string) =>
    api.get<any, ApiResponse<{ report: any }>>(`/report/${id}`),

  getByProjectId: (projectId: string) =>
    api.get<any, ApiResponse<{ reports: any[] }>>(`/report/project/${projectId}`),

  delete: (id: string) =>
    api.delete<any, ApiResponse>(`/report/${id}`),

  getTemplates: () =>
    api.get<any, ApiResponse<{ templates: any[] }>>('/report/templates'),
  ...
  createTemplate: (templateData: {
    ...
    api.post<any, ApiResponse<{ template: any }>>('/report/templates', templateData),

  updateTemplate: (id: string, templateData: any) =>
    api.put<any, ApiResponse<{ template: any }>>(`/report/templates/${id}`, templateData),

  deleteTemplate: (id: string) =>
    api.delete<any, ApiResponse>(`/report/templates/${id}`),

  setDefaultTemplate: (templateId: string) =>
    api.post<any, ApiResponse>(`/report/templates/set-default`, { template_id: templateId }),

  preview: (params: {
    content: string
    format: 'html' | 'markdown'
  }) =>
    api.post<any, ApiResponse<{ preview: string }>>('/report/preview', params),
  ...
  resumeGeneration: (reportId: string) =>
    api.patch<any, ApiResponse>(`/report/${reportId}/resume`),

  stopGeneration: (reportId: string) =>
    api.patch<any, ApiResponse>(`/report/${reportId}/stop`),
}

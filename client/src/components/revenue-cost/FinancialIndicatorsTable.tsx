...
const FinancialIndicatorsTable: React.FC<FinancialIndicatorsTableProps> = ({
  ...
  const { context, revenueItems, productionRates, costConfig, revenueTableData, costTableData, profitDistributionTableData, loanRepaymentTableData, setFinancialIndicators } = useRevenueCostStore()
  ...
  // 缓存的财务指标计算函数 - 修复：在useEffect中计算，不在渲染期间更新状态
  const useCachedFinancialIndicators = () => {
    ...
    // 【调试日志】标记计算时间
    console.log('[FinancialIndicators] 重新计算财务指标:', new Date().toISOString());
  ...
  }, [context, preTaxRate, postTaxRate, revenueTableData, costTableData, profitDistributionTableData]);

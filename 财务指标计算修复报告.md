...
## 🔄 2024-12-27 更新：数据源一致性修复

### 问题描述
用户反馈："财务计算指标表，指标计算所需的基础数据不正确，应该使用导出excel的数据"

### 修复内容
1. **统一数据源依赖**：在`useCachedFinancialIndicators`函数中添加了更多的数据源依赖，确保当表格数据变化时，财务指标会重新计算
2. **修复动态现金流量计算公式**：确保`generateCashFlowTableData`函数中的动态现金流量计算与Excel导出使用相同的公式
3. **优化财务指标计算函数**：改进`calculateFinancialIndicators`函数，确保使用与Excel导出相同的计算逻辑

### 具体修改
```typescript
// 在useCachedFinancialIndicators中添加数据源依赖
...
// 再计算所得税后净现金流量（动态）= C-D/(1+E)^B
const discountFactor = Math.pow(1 + postTaxRateDecimal, year);
const dynamicValue = dynamicPreTaxCashFlow - adjustedIncomeTax / discountFactor;
```

### 测试文件
创建了`test-financial-indicators-consistency.html`用于验证修改后的财务指标计算与Excel导出数据的一致性。

### 预期结果
修改后，财务指标表中显示的所有数值应该与导出的Excel文件中的对应数值完全一致。
